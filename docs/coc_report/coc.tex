...
\usepackage{float} % ??? QQQ -- for [H] on figure
...
This simulation model was applied to a variety of background sets to produce prey camouflage patterns suited to those environments. Examples from several runs are shown in the Supplemental Materials.
...
Names and descriptions for the sets of background images used in this paper, see section \nameref{subsec:background_sets}. Each set is composed of several photographs of a similar natural scene. \textbf{(Note to reviewers, these images will be made available (along with source code, other data and development documentation) if paper is accepted for publication.)}
\par
\hspace*{1cm}
