...
This work builds on \citet{reynolds_iec_2011} by replacing the human predator there with an evolving population of procedural predators, which \jargon{hunt} using a learning vision model. \citet{harrington_coevolution_2014} also used coevolution between prey and predators. Their predators detected prey with a multiscale convolution filter whose weights were evolved with a genetic algorithm.
\par
...
\par
As used here, “similarity” is meant to suggest image features have a \jargon{stationary distribution}, that different patches (e.g. of a size comparable to prey) have the same statistical distribution of color and frequency. When background sets are “less stationary” (e.g. large areas of single color) it becomes \jargon{harder} for evolution to find good camouflage. In these cases, better results can be had with an “extra large” run. See Section \ref{sec:discussion}.
\par
...
\par
An important next step is to better understand the failure modes of this simulation. Some environments (background sets) are \textit{easy} while others are \textit{hard}. It seems that easy environment are more “stationary.” As mentioned in Section\ref{sec:discussion}, hard backgrounds can sometimes be addressed with changing a random seed or using an “extra large” simulation. These suggest that hard backgrounds have a lower liklihood of success. Perhaps the standard size simulation used here is a bit underpowered?
\par
...
\begin{figure*}
    ...
    \caption{Four tournament images from run bean\_soup\_mix\_20221231\_1317. (“Extra large” run.)}
    \Description{QQQ.}
...
\begin{figure*}
    ...
    \caption{Four tournament images from run backyard\_oak\_20230113\_2254. (“Extra large” run.)}
    \Description{QQQ.}
...
\begin{figure*}
    ...
    \caption{Four tournament images from run mbta\_flowers\_20230105\_1114. (“Extra large” run.)}
    \Description{QQQ.}
...
\begin{figure*}
    ...
    \caption{Four tournament images from run yellow\_flower\_on\_green\_20221217\_1826. This was a notoriously “hard” non-stationary test case.}
    \Description{QQQ.}
...
\hline
backyard\_oak &
    under canopy of California live oak (Quercus agrifolia) &
...
\begin{figure}[H]
    ...
    \caption{Four tournament images from run redwood\_leaf\_litter\_20230115\_1730.}
    \Description{QQQ.}
...
Similar to Fig. \ref{fig:predator_responses}, these examples from run tiger\_eye\_beans\_20220903\_1401 show the prediction output of all three predators in each tournament. The crosshairs are ordered by least “aim error” with the best drawn in black and white, seconds in black and green, and worst in black and red. From left to right: (a) all three predators miss all three prey, (b) best is near center of one prey, second is off center but still inside another prey, and third fails to find any prey, (c) all three succeed, best inside one prey, the other two inside another prey, and (d) all three predators select the same prey which has cartoonishly conspicuous coloration.
...
\begin{figure}[H]
    ...
    \caption{Four tournament images, plus predator responses as crosshairs, from run tiger\_eye\_beans\_20220903\_1401.}
    \Description{QQQ.}
