...
// TODO TEST new version with closed form expression for contraction

// Modifies the given texture within a disk of "radius" around "center", doing a
// "fisheye" expansion of the center of the disk (when center_magnification > 1)
// or a contraction (when center_magnification < 1).
class StretchSpot2 : public Operator
{
public:
    StretchSpot2(float _center_magnification,
                float radius,
                Vec2 _center,
                const Texture& _texture) :
        center_magnification(_center_magnification),
    ...
    float inverseRemapper(float rr) const
    {
        ...
        // TODO maybe this?
//        float factor = 10; // TODO Saturday's guess (ave pixel error = 0.0387)
        
        // TODO comparing old and new mag=0.2 radius=0.6
//        float factor = 5;  // worse
//        float factor = 12;  // ...  (ave pixel error = 0.0179)
//        float factor = 13;  // ...  (ave pixel error = 0.0107) ****
//        float factor = 14;  // ...  (ave pixel error = 0.0108) ****
//        float factor = 15;  // better  (ave pixel error = 0.0131)
//        float factor = 16;  // ...  (ave pixel error = 0.0167)
//        float factor = 20;  // sameish  (ave pixel error = 0.0315)
        
        // TODO comparing old and new mag=0.5 radius=0.6
//        float factor = 2;  // ave pixel error = 0.0567
//        float factor = 5;  // ave pixel error = 0.0163
//        float factor = 7.5;  // ave pixel error = 0.0106 ****
//        float factor = 10;  // ave pixel error = 0.0210
        
        // TODO comparing old and new mag=0.25 radius=1
//        float factor = 5;  // ave pixel error = 0.3492
//        float factor = 8;  // ave pixel error = 0.1580
//        float factor = 9;  // ave pixel error = 0.1029
//        float factor = 10;  // ave pixel error = 0.0556
//        float factor = 11;  // ave pixel error = 0.0475 ****
//        float factor = 12;  // ave pixel error = 0.0591
//        float factor = 16;  // ave pixel error = 0.1347
        
        // TODO comparing old and new mag=0.1 radius=1
//        float factor = 2;  // ave pixel error = 0.3476
//        float factor = 5;  // ave pixel error = 0.3262
//        float factor = 11;  // ave pixel error = 0.3274
//        float factor = 15;  // ave pixel error = 0.3270
//        float factor = 20;  // ave pixel error = 0.2421
//        float factor = 30;  // ave pixel error = 0.0608 ****
        
        
//        float factor = 3 * outer_radius / center_magnification;
//        // first  test, ave pixel error = 0.0514
//        // second test, ave pixel error = 0
//        // third  test, ave pixel error = 0.0337
//        // fourth test, ave pixel error = 0.0591
//        // fifth  test, ave pixel error = 0.0608

//        float factor = 5 * outer_radius / center_magnification;
//        // first  test, ave pixel error = 0.0131856
//        // second test, ave pixel error = 0
//        // third  test, ave pixel error = 0.0079
//        // fourth test, ave pixel error = 0.1975
//        // fifth  test, ave pixel error = 0.2246

//        float factor = 6 * outer_radius / ( 2 * center_magnification);
//        // first  test, ave pixel error =0.0514
//        // second test, ave pixel error = 0
//        // third  test, ave pixel error = 0.0337
//        // fourth test, ave pixel error = 0.0591
...
//        float factor = (15 * center_magnification) / (2 * outer_radius);
//        // first  test, ave pixel error =
//        // second test, ave pixel error = 0
//        // third  test, ave pixel error =
//        // fourth test, ave pixel error =
//        // fifth  test, ave pixel error =
...
//        return ((curve - extra) / center_magnification)  + 1;
    ...
    void ifNeededInitializeInverseLUT()
    {
        if (center_magnification < 1)
        {
            inverse_lut = std::make_shared<std::vector<float>>(lutSize());
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            // init all slots in table to -1 to check for filling
            for (int k = 0; k < lutSize(); k++) inverse_lut->at(k) = -1;
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            float lastQ = 0;
            int i = 0;
            inverse_lut->at(0) = 0;
            ...
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            for (float r = 0; r <= 1; r = r + (inverseRes * 0.01))
//            for (float r = 0; r <= 1; r = r + (inverseRes * 0.1))
//            for (float r = 0; r <= 1; r = r + (inverseRes * 0.5))
//            for (float r = 0; r <= 1; r = r + inverseRes)
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            {
                float q = r * remapper (r);
                float d = q - lastQ;
            ...
            {
                if (inverse_lut->at(k) == -1)
                {
                    std::cout << k << ": " << inverse_lut->at(k) << std::endl;
                }
            }
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
