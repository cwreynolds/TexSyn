...
//            for (auto& spot : spots)
//            {
//                auto tiled_1d = [&](float x)
//                    { return fmod_floor(x + tile_half, tile_size) - tile_half; };
//    //            Vec2 tiled_point(fmod_floor(position.x() + 5, tile_size) - 5,
//    //                             fmod_floor(position.y() + 5, tile_size) - 5);
//                Vec2 tiled_point(tiled_1d(position.x()), tiled_1d(position.y()));
//                // Distance from sample position to spot center.
//    //            float d = (position - spot.position).length();
...
//
//            bool in_center_tile = (between(position.x(), -tile_half, tile_half) &&
//                                   between(position.y(), -tile_half, tile_half));
//            return (in_center_tile ?
//                    Color::gray(gray_level) :
//                    Color(0.6, 0.6, 0.8) * gray_level);
//        }
//    private:
...
//        }
//        const float tile_size = 10;
//        const float tile_half = tile_size / 2;
//    //    const int move_count = 20;
//    //    const int move_count = 100;
//        const int move_count = 60;
//        std::vector<Dot> spots;
...
//
//    //    Vec2 nearest_of_9_tiles(Dot& ax, Dot& b)
//    //    Vec2 nearest_of_9_tiles(Vec2 reference_point, Vec2 spot_center)
//        Vec2 nearestByTiling(Vec2 reference_point, Vec2 spot_center)
//        {
//            // Nearest distance and point.
//            float nd = std::numeric_limits<float>::infinity();
//    //        Vec2 np = b.position;
...
//                for (float y : {-tile_size, 0.0f, tile_size})
//                {
//                    Vec2 new_b = spot_center + Vec2(x, y);
//                    float d = (reference_point - new_b).length();
//                    if (nd > d)
//                    {
//                        nd = d;
//                        np = new_b;
...
//    //
//
//                Vec2 tiled_point(fmod_floor(position.x() + half, tile_size) - half,
...
//                        }
//                    }
//    //                // Clip "a"s position to be within tile, clear "no_move"
//    //                Vec2 a_clipped = Vec2(clip(a.position.x(), -half, half),
//    //                                      clip(a.position.y(), -half, half));
//    //                if (a.position != a_clipped)
//    //                {
//    //                    no_move = false;
//    //                    a.position = a_clipped;
//    //                }
//
//
//                    // Clip "a"s position to be within tile, clear "no_move"
...
//    //            Dot spot(frandom2(min_radius, max_radius),
//    //                     Vec2(frandom2(-half, half), frandom2(-half, half)));
//    //            float radius = interpolate(sq(frandom01()), min_radius, max_radius);
//                float radius = interpolate(std::pow(frandom01(), 3),
//                                           min_radius, max_radius);
//                Dot spot(radius, Vec2(frandom2(-half, half), frandom2(-half, half)));
//
//                spots.push_back(spot);
//                total_area += spot.area();
//            }
...
private:
    ...
    void generateSpots()
    {
        ...
        float half = tile_size / 2;
        RandomSequence rs(
                          uint32_t(spot_density) ^
                          uint32_t(min_radius) ^
                          uint32_t(max_radius) ^
                          uint32_t(soft_edge_width)
                          
                          );
        while (total_area < (spot_density * sq(tile_size)))
        {
//            float i = std::pow(frandom01(), 3);
//            float i = std::pow(frandom01(), 4);
            float i = std::pow(rs.frandom01(), 4);
            float radius = interpolate(i, min_radius, max_radius);
//            Dot spot(radius, Vec2(frandom2(-half, half), frandom2(-half, half)));
            Dot spot(radius,
                     Vec2(rs.frandom2(-half, half), rs.frandom2(-half, half)));
            spots.push_back(spot);
            total_area += spot.area();
        }
        // Move spots away from regions of overlap, repeat move_count times.
...
//                            float adjust = 0.2 * (move_count - i) / move_count;
//                            float adjust = 0.5 * (move_count - i) / move_count;
